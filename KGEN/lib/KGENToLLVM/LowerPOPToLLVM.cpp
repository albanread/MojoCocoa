...
struct ConvertPOPGlobalAlloc : public ConvertSymbolOpToLLVM<GlobalAllocOp> {
  ...
  LogicalResult matchAndRewrite(GlobalAllocOp op, GlobalAllocOpAdaptor adaptor,
    ...
    // COCOA: a fixed-name host global (not GPU_SHARED, no initializer) is a
...
struct ConvertExternPointerSymbol
  ...
  LogicalResult matchAndRewrite(ExternPointerSymbolOp op,
    ...
    // COCOA: reuse an existing declaration for this external name instead
    ...
    if (!global) {
      b.clearInsertionPoint();
      // COCOA: dso_local=false. An external symbol may live in another
      ...
      // and the linker reject it. (The x86-64 fork hit this as "invalid use
      // of rip-relative addressing"; on arm64 it is the equivalent direct
      // ADRP/ADD page-relative form instead of an ADRP+LDR through the GOT.)
