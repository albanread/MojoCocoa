...
GENERATED by tools/galaxigans-art.py from the BASIC source, then committed:
the conversion runs once, not at every build, and the result is art a person
can read and edit. Several of these were DRAWN in the original -- ellipses,
triangles and filled circles into the definition's pixel buffer -- and
`define_sprite` takes text, so the drawing was done offline and the pixels
are what survives.
...
comptime SPRITE_COUNT = 12


def define_all(mut ctx: DeviceContext, mut sprites: Sprites) raises -> List[Int]:
    """Every definition and its palette, in the BASIC's own order. Returns
    the handles, so a game indexes them by the constants below."""
    var ids = List[Int]()
    ...
    # BANNER -- 32x14, 1 frame
    let banner_ID = sprites.define_sprite(ctx, BANNER)
    sprites.sprite_rgb(banner_ID, 1, 20, 20, 20)
    sprites.sprite_rgb(banner_ID, 2, 200, 200, 255)
    sprites.sprite_rgb(banner_ID, 3, 120, 150, 255)
    sprites.sprite_rgb(banner_ID, 4, 255, 120, 60)
    sprites.sprite_rgb(banner_ID, 5, 255, 255, 255)
    ids.append(banner_ID)

    # SAUCER -- 16x16, 1 frame
    let saucer_ID = sprites.define_sprite(ctx, SAUCER)
    sprites.sprite_rgb(saucer_ID, 1, 0, 0, 0)
    sprites.sprite_rgb(saucer_ID, 2, 220, 60, 220)
    sprites.sprite_rgb(saucer_ID, 3, 255, 255, 255)
    sprites.sprite_rgb(saucer_ID, 4, 0, 255, 255)
    sprites.sprite_rgb(saucer_ID, 5, 255, 120, 60)
    ids.append(saucer_ID)

    # BEE_FLAP -- 16x16, 2 frames
    let bee_flap_ID = sprites.define_sprite(ctx, BEE_FLAP_F0)
    _ = sprites.add_frame(ctx, bee_flap_ID, BEE_FLAP_F1)
    sprites.sprite_rgb(bee_flap_ID, 1, 0, 0, 0)
    sprites.sprite_rgb(bee_flap_ID, 2, 220, 0, 0)
    sprites.sprite_rgb(bee_flap_ID, 3, 255, 255, 0)
    sprites.sprite_rgb(bee_flap_ID, 4, 120, 120, 120)
    ids.append(bee_flap_ID)

    # BOSS_FLAP -- 16x16, 2 frames
    let boss_flap_ID = sprites.define_sprite(ctx, BOSS_FLAP_F0)
    _ = sprites.add_frame(ctx, boss_flap_ID, BOSS_FLAP_F1)
    sprites.sprite_rgb(boss_flap_ID, 1, 0, 0, 0)
    sprites.sprite_rgb(boss_flap_ID, 2, 60, 100, 255)
    sprites.sprite_rgb(boss_flap_ID, 3, 255, 230, 0)
    sprites.sprite_rgb(boss_flap_ID, 4, 150, 200, 255)
    sprites.sprite_rgb(boss_flap_ID, 5, 220, 0, 0)
    ids.append(boss_flap_ID)

    # DIVER -- 16x16, 3 frames
    let diver_ID = sprites.define_sprite(ctx, DIVER_F0)
    _ = sprites.add_frame(ctx, diver_ID, DIVER_F1)
    _ = sprites.add_frame(ctx, diver_ID, DIVER_F2)
    sprites.sprite_rgb(diver_ID, 1, 0, 0, 0)
    sprites.sprite_rgb(diver_ID, 2, 180, 180, 180)
    sprites.sprite_rgb(diver_ID, 3, 100, 255, 100)
    sprites.sprite_rgb(diver_ID, 4, 255, 255, 255)
    sprites.sprite_rgb(diver_ID, 5, 255, 0, 255)
    ids.append(diver_ID)
    return ids^
...
comptime BANNER_SLOT = 7
comptime SAUCER_SLOT = 8
comptime BEE_FLAP_SLOT = 9
comptime BOSS_FLAP_SLOT = 10
comptime DIVER_SLOT = 11
