...
GENERATED by tools/galaxigans-art.py, then committed: the conversion runs
once, not at every build, and the result is art a person can read and edit.
Half of it was DRAWN in the original -- ellipses, triangles and filled
circles into the definition's pixel buffer -- and `define_sprite` takes
text, so the drawing was done offline and the pixels are what survives.
