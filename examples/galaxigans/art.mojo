...
def define_all(mut ctx: DeviceContext, mut sprites: Sprites) raises -> List[Int]:
    ...
    let player_ID = sprites.define_sprite(ctx, PLAYER)
    ...
    # BEE -- 16x16, 1 frame
    let bee_ID = sprites.define_sprite(ctx, BEE)
    ...
    # BOSS -- 24x20, 1 frame
    let boss_ID = sprites.define_sprite(ctx, BOSS)
    ...
    # BULLET -- 4x8, 1 frame
    let bullet_ID = sprites.define_sprite(ctx, BULLET)
    ...
    # BOMB -- 4x8, 1 frame
    let bomb_ID = sprites.define_sprite(ctx, BOMB)
    sprites.sprite_rgb(bomb_ID, 1, 255, 120, 0)
    sprites.sprite_rgb(bomb_ID, 2, 255, 60, 60)
    ids.append(bomb_ID)

    # STAR -- 4x4, 1 frame
    let star_ID = sprites.define_sprite(ctx, STAR)
    sprites.sprite_rgb(star_ID, 1, 200, 200, 255)
    ids.append(star_ID)

    # EXPLOSION -- 24x24, 6 frames
    let explosion_ID = sprites.define_sprite(ctx, EXPLOSION_F0)
    _ = sprites.add_frame(ctx, explosion_ID, EXPLOSION_F1)
    _ = sprites.add_frame(ctx, explosion_ID, EXPLOSION_F2)
    _ = sprites.add_frame(ctx, explosion_ID, EXPLOSION_F3)
    _ = sprites.add_frame(ctx, explosion_ID, EXPLOSION_F4)
    _ = sprites.add_frame(ctx, explosion_ID, EXPLOSION_F5)
    ...
    let saucer_ID = sprites.define_sprite(ctx, SAUCER)
    ...
    let butterfly_ID = sprites.define_sprite(ctx, BUTTERFLY)
    ...
    let scorpion_ID = sprites.define_sprite(ctx, SCORPION_F0)
    _ = sprites.add_frame(ctx, scorpion_ID, SCORPION_F1)
    ...
    let blue_bee_ID = sprites.define_sprite(ctx, BLUE_BEE_F0)
    _ = sprites.add_frame(ctx, blue_bee_ID, BLUE_BEE_F1)
    ...
    # MOTH -- 16x16, 3 frames
    let moth_ID = sprites.define_sprite(ctx, MOTH_F0)
    _ = sprites.add_frame(ctx, moth_ID, MOTH_F1)
    _ = sprites.add_frame(ctx, moth_ID, MOTH_F2)
