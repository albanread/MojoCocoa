...
struct Game(Movable):
    ...
    # ── the capture boss ─────────────────────────────────────────────────
    def step_boss(mut self, field: IndexedPane, deck: P) raises:
    ...
    def cascade_beam(mut self, field: IndexedPane) raises:
    ...
    # ── drawing ──────────────────────────────────────────────────────────
    def draw(mut self, mut sprites: Sprites, field: IndexedPane, mut hud: TextOverlay, hud_key: Int) raises:
...
def main() raises:
    ...
    var hud = TextOverlay(pane.device, FIELD_W, FIELD_H)
    var game = Game(pane.ctx, sprites)

    let headless = getenv("GAMEPANE_FRAMES").byte_length() > 0
    let auto = headless or getenv("GDX_AUTOPLAY").byte_length() > 0
    var deck = deck_new()
    let unit = start_audio(deck)
    game.new_game(sprites)
    game.trace = getenv("GDX_TRACE").byte_length() > 0
    game.pacifist = getenv("GDX_PACIFIST").byte_length() > 0
    ...
    let start_wave = parse_int(getenv("GDX_WAVE"))
    if start_wave > 1:
    ...
    while pane.pump():
        ...
        with autoreleasepool():
            ...
            cosmos.render(f)           # layer 0: Clear -- the cosmos
            field.render(f)            # layer 1: bullets, bombs, sparks, the beam
            sprites.render(f, 0.0, 0.0, Float64(FIELD_W), Float64(FIELD_H))
            hud.render(f)              # layer 3: the text
            pane.end_frame(f)
    stop_tune_gm()
    stop_audio(unit)
    deck_free(deck)
