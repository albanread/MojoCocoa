...
def mojo_bin() -> String:
    ...
    return String(MOJO_BIN_DEFAULT)
...
def build_menu(app: ObjCObject, actions: ObjCObject):
    ...
    var r2 = menu_item(
        String("Run on the GPU"), "pgRunGPU:", String("R"), 131072 + 1048576
    )
...
def main() raises:
    with autoreleasepool():
        ...
        highlight_editor()
        append_output(
            String("Mojo Mac Playground — ⌘R to run, ⇧⌘R on the GPU.\n"),
            OUT_STATUS,
        )
