# AsyncRT C ABI signatures, recovered from Mojo call sites

Split out of ABI-NOTES.md when that became a generated capability table. This
half is NOT generated: the prototypes were recovered by reading the
`# const char *AsyncRT_...` comments beside each `external_call` in
device_context.mojo, and there is no machine-readable source to derive them
from. Kept because reconstructing them again would be a day's work.

For what each symbol actually DOES when called, see ABI-NOTES.md.
