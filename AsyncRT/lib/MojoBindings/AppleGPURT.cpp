//===----------------------------------------------------------------------===//
// AppleGPURT — the MacVegaFork implementation of the AsyncRT device ABI.
//
...
// Phase 2b adds the Metal backend for Apple Silicon GPUs.
//
...
struct VRBuffer : RefCounted {
  ...
  AGMetalBuf *mtl = nullptr;     // set when the owning context is metal
...
AsyncRT_DeviceContext_create(const DeviceContext **result, const char *api,
  ...
  if (want == "cpu" || want == "default") {
    ...
    ctx->api = "cpu";
    ctx->arch = "cpu";
    ctx->name = "AppleGPURT cpu:0 (Xeon W, x86-64 Mac Pro)"; // test_smoke checks for a lowercase "cpu" substring
    *result = ctx;
    return VR_OK;
  }
  if (want == "metal" || want == "gpu") {
    char name[256], arch[64];
    AGMetalCtx *mctx = nullptr;
    if (const char *err =
            AppleGPUMetal_createContext(&mctx, id, name, sizeof(name), arch,
                                      sizeof(arch)))
      return err;
    auto *ctx = new VRContext();
    ctx->id = id;
  ...
  return vrErrorf("AppleGPURT: device api '%s' is not available (have: cpu, metal)",
                  want.c_str());
}
...
extern "C" int32_t AsyncRT_DeviceContext_numberOfDevices(const char *kind) {
  ...
  if (k == "cpu" || k == "default")
    return 1;
  if (k == "gpu" || k == "metal")
    return AppleGPUMetal_deviceCount();
  return 0;
}
...
AsyncRT_DeviceContext_getAttribute(int *result, const DeviceContext *ctx,
  ...
  if (ctx->metal && AppleGPUMetal_getAttribute(ctx->metal, attr, result) == 0)
    return VR_OK;
  switch (attr) {
  ...
  default:
    *result = 0;
    return vrErrorf(
        "AppleGPURT: DeviceContext_getAttribute(%d) not implemented for api '%s'",
        attr, ctx->api.c_str());
  }
}
...
AsyncRT_DeviceContext_getMemoryInfo(const DeviceContext *ctx, size_t *freeMem,
                                    size_t *total) {
  if (ctx->metal)
    return AppleGPUMetal_memInfo(ctx->metal, freeMem, total);
  uint64_t mem = 0;
...
extern "C" const char *AsyncRT_DeviceContext_synchronize(const DeviceContext *ctx) {
  if (ctx->metal)
    return AppleGPUMetal_synchronize(ctx->metal);
...
extern "C" const char *AsyncRT_DeviceContext_enableAllPeerAccess() {
  return vrErrorf("AppleGPURT: peer access not supported");
}
...
AsyncRT_DeviceContext_createBuffer_async(const DeviceBuffer **result,
  ...
  if (ctx->metal) {
    AGMetalBuf *mbuf = nullptr;
    void *devAddr = nullptr;
    if (const char *err = AppleGPUMetal_createBuffer(&mbuf, &devAddr, ctx->metal,
                                                   bytes, /*host=*/false))
      return err;
  ...
  if (!p)
    return vrErrorf("AppleGPURT: allocation of %zu bytes failed", bytes);
...
AsyncRT_DeviceContext_createHostBuffer(const DeviceBuffer **result,
  ...
  if (!p)
    return vrErrorf("AppleGPURT: host allocation of %zu bytes failed", bytes);
...
AsyncRT_DeviceBuffer_createSubBuffer(const DeviceBuffer **result,
  ...
  if (offBytes + bytes > buf->bytes)
    return vrErrorf("AppleGPURT: sub-buffer [%zu, %zu) exceeds parent size %zu",
                    offBytes, offBytes + bytes, buf->bytes);
  if (buf->mtl) {
    AGMetalBuf *msub = nullptr;
    void *devAddr = nullptr;
    if (const char *err = AppleGPUMetal_createSubBuffer(&msub, &devAddr,
                                                      buf->mtl, offBytes,
                                                      bytes))
      return err;
...
extern "C" const char *AsyncRT_DeviceBuffer_hostPtr(void **result,
  ...
  if (buffer->mtl) {
    *result = AppleGPUMetal_hostPtr(buffer->mtl); // null for private buffers
    return VR_OK;
  }
...
AsyncRT_DeviceContext_setMemory_async(const DeviceContext *,
  ...
  default:
    return vrErrorf("AppleGPURT: setMemory with element size %zu unsupported",
                    val_size);
  }
}
...
#define VR_STUB_ERR(name)                                                      \
  extern "C" const char *name() {                                              \
    return vrErrorf("AppleGPURT: %s not implemented (phase 2)", #name);            \
  }
...
extern "C" const char *AsyncRT_DeviceContext_enqueueFunctionDirect(
  ...
  (void)attributes;
  if (numAttributes)
    return vrErrorf("AppleGPURT: launch attributes not supported yet (%u given)",
                    numAttributes);
  if (!ctx->metal)
    return vrErrorf("AppleGPURT: kernel launch requires a metal context (api is "
                    "'%s')",
                    ctx->api.c_str());
  ...
  if (argSizes == nullptr && argCount > 0 && args != nullptr) {
    ...
    if (!mv || !mv->addrs || !mv->sizes || !mv->isDevicePtr)
      return vrErrorf("AppleGPURT: malformed MetalEnqueueFunctionArgs (null "
                      "fields); launches without sizes must use the wrapper "
                      "protocol");
    return AppleGPUMetal_launch(ctx->metal, func->mtl, grid, block,
  ...
  return AppleGPUMetal_launch(ctx->metal, func->mtl, grid, block, sharedMemBytes,
...
AsyncRT_DeviceFunction_getAttribute(int32_t *result, const DeviceFunction *fn,
                                    int32_t attr_code) {
  *result = 0;
  return vrErrorf("AppleGPURT: DeviceFunction_getAttribute(%d) not implemented",
                  attr_code);
}
...
AsyncRT_DeviceContext_metal_device(void **result, const DeviceContext *ctx) {
  if (!ctx->metal)
    return vrErrorf("AppleGPURT: metal_device on non-metal context");
...
AsyncRT_DeviceContext_HtoD_async_sized(const DeviceContext *ctx, void *dst,
  ...
  if (ctx->metal)
    return AppleGPUMetal_copyRawHtoD(ctx->metal,
...
AsyncRT_DeviceContext_DtoH_async_sized(const DeviceContext *ctx, void *dst,
                                       const void *src, size_t bytes) {
  if (ctx->metal)
    return AppleGPUMetal_copyRawDtoH(ctx->metal, dst,
...
AsyncRT_DeviceContext_maxSingleAllocationSize(const DeviceContext *ctx,
                                              size_t *result) {
  if (ctx->metal) {
    *result = AppleGPUMetal_maxAlloc(ctx->metal); // MTLDevice maxBufferLength
    return VR_OK;
  }
