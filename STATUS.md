...
| Examples and the game engine | Passing | `./tools/check-examples.sh`: 24 passed, 0 failed, 1 skipped (`life-python`, needs pygame-ce); `./tools/check-gamepane.sh`: 17/17 (6 Sep 2026, after the D21 fix -- the build before it failed every GPU example, and only these two scripts showed it). Two games ship: `galaxigans` and `galaxigans-deluxe`. |
...
| Loop unrolling for AIR (D24) | Shipped, gated | LLVM's unroller at its O3 position for AIR, single-block loops ≤128 instructions, partial threshold 1024, loop metadata stripped: FMA-chain bench at the M4's 3.6 TFLOP/s peak from four chains (was 1,321), SRAM matmul +12%, no bench regressed; corpus sweep: 0 regressions on the 91-target baseline. 7 Sep 2026. |
| Register matmul vs upstream (D7) | Resolved: at parity | Fully unrolled 64x64/4x4 kernel at 2048³ on the M4: 870 → 970 GFLOP/s with SLP vectorization off for AIR (upstream 956, Apple's own compiler from MSL 951; rolled kernel 958). Partial unrolling, still opt-in, lifts the FMA-chain bench to the machine's 3.6 TFLOP/s peak. 6 Sep 2026, `oracles/findings/d7-unrolled-matmul.md`. |
| AppleGPURT dispatch cost | At Metal's floor | Empty dependent dispatch 0.9 µs batched (Metal's own one-encoder floor 1.0 µs; was 4.2), `compile_function` cached at 0.8 µs (was 17.8), host round trip ~150 µs and Apple's. 6 Sep 2026, D9 table. |
